...
export default function Index() {
  ...
  if (screen === "welcome") {
    return <WelcomeScreen onSelect={(r) => { setRole(r); setScreen("app"); }} />;
  }

  return (
    <AppShell role={role} tab={tab} setTab={setTab} onRoleChange={() => setScreen("welcome")} />
  );
}
...
function WelcomeScreen({ onSelect }: { onSelect: (r: Role) => void }) {
  return (
    <div className="min-h-screen mesh-bg flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center relative z-10">
        ...
        <div className="w-full max-w-xs space-y-3 slide-up" style={{ animationDelay: "0.1s" }}>
          <RoleCard
            ...
            emoji="🏠"
            title="Я заказчик"
            desc="Создаю заявки на вывоз мусора"
            accent="lime"
            onSelect={onSelect}
          />
          <RoleCard
            ...
            emoji="⚡"
            title="Я исполнитель"
            desc="Выполняю заказы и зарабатываю"
            accent="purple"
            onSelect={onSelect}
          />
        </div>
      ...
      <p className="text-center text-xs text-muted-foreground/40 pb-6 relative z-10">
        МусорОфф v1.0 — для молодых и активных
      </p>
    </div>
  );
}
...
function RoleCard({
  ...
  return (
    <button
      ...
      style={{
        background: isLime
          ? "linear-gradient(135deg, rgba(170,255,0,0.08), rgba(170,255,0,0.02))"
          : "linear-gradient(135deg, rgba(187,68,255,0.08), rgba(187,68,255,0.02))",
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <p
            className="font-display font-900 text-base"
            style={{ color: isLime ? "#aaff00" : "#bb44ff" }}
          >
            {title}
          </p>
...
function AppShell({
  ...
  return (
    <div className="min-h-screen mesh-bg flex flex-col max-w-md mx-auto relative">
        ...
        <div className="flex items-center justify-between h-14">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-900 text-lg">
              Мусор<span className="text-lime">Офф</span>
            </span>
          </div>
          ...
          >
            ...
            <span
              ...
              style={{ color: role === "customer" ? "#aaff00" : "#bb44ff" }}
            >
      ...
      <nav
        className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md glass border-t border-white/8 z-40"
      >
        <div className="flex">
          {TABS.map((t) => {
              ...
              >
                {isActive && (
                  <span
                    ...
                    style={{ background: "linear-gradient(90deg, #aaff00, #66dd00)" }}
                  />
                )}
                <Icon
                  name={t.icon}
                  size={20}
                  className={isActive ? "text-lime" : "text-muted-foreground"}
                />
                <span
